...
exports.createFormLink = async (req, res) => {
  try {
    ...
    // Verify teacher is assigned to this class/division using teacher_id
...
exports.submitFormViaLink = async (req, res) => {
  ...
  try {
    ...
    // -----------------------------
    // Get division_id using teacher_id
    // -----------------------------
    ...
    // =====================================================
    // DUPLICATE CHECK (ONLY IF roll_number EXISTS)
    // =====================================================
    if (roll_number !== undefined && roll_number !== null && roll_number !== '') {
      ...
      if (duplicateCheck.length > 0) {
        return res.status(409).json({
          success: false,
          message: 'Form already submitted for this roll number and class',
        });
      }
    }
    ...
    if (data) {
      try {
        await linkStudentToParentAccount(data);
      } catch (linkErr) {
...
exports.getFormLinks = async (req, res) => {
  try {
    ...
    // Get divisions assigned to this teacher using teacher_id
    console.log('🔍 Looking for divisions assigned to teacher_id:', teacherId);
    const divisions = await sequelize.query(
      `SELECT class_name, division_name
             FROM divisions
